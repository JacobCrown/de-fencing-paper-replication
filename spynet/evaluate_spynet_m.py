...
class EvalConfig:
    ...
    BATCH_SIZE: int = 16
    NUM_SAMPLES_TO_EVALUATE: int = 100
    ...
    NUM_WORKERS: int = 0  # 0 for main process, helps with debugging on Windows
...
def worker_init_fn(worker_id):
    ...
    # Use a fixed seed + worker_id to ensure reproducibility across runs,
    # but different sequences for different workers if desired.
    # For strict same data order for all models, the base seed (0 here) is most important.
    seed = 0 + worker_id
...
def evaluate_single_model(config, model_path_to_evaluate, results_summary):
    ...
    # Set seeds for determinism before creating dataset and dataloader for this model evaluation
    torch.manual_seed(0)
    random.seed(0)
    np.random.seed(0)

    # 1. Model
    model = SPyNetModified(model_name=config.VANILLA_SPYNET_NAME, pretrained=False)

    if not os.path.exists(model_path_to_evaluate):
        print(f"ERROR: Model path does not exist: {model_path_to_evaluate}")
        results_summary[os.path.basename(model_path_to_evaluate)] = {
            "aepe": float("inf"),
    ...
    try:
        ...
        if model_state_dict_to_load:
            ...
            print(
                f"Loaded model weights from {os.path.basename(model_path_to_evaluate)}."
            )
        else:
            print(
                f"Could not identify model state_dict in {os.path.basename(model_path_to_evaluate)}"
            )
            results_summary[os.path.basename(model_path_to_evaluate)] = {
                "aepe": float("inf"),
    ...
    try:
        ...
        results_summary[os.path.basename(model_path_to_evaluate)] = {
            "aepe": float("inf"),
            "error": f"Dataset creation error: {e}",
        }
        return
    ...
    eval_dataset_subset = torch.utils.data.Subset(
        eval_dataset, list(range(num_samples))
    )
    ...
    with torch.inference_mode():
        for i, batch_data in enumerate(eval_loader):
            if (
                config.NUM_SAMPLES_TO_EVALUATE is not None
                and i * config.BATCH_SIZE >= config.NUM_SAMPLES_TO_EVALUATE
            ):
                break
            ...
            if (
                ...
                and input1_rgbm.size(0)
                >= config.NUM_SAMPLES_TO_VISUALIZE  # Ensure batch is large enough
            ):
                # print(f"Visualizing first batch for model {os.path.basename(model_path_to_evaluate)}...")
                model_name_suffix = (
                    "_" + os.path.splitext(os.path.basename(model_path_to_evaluate))[0]
                )
    ...
    if num_samples_processed_count > 0:
        ...
        print(
            f"Model: {os.path.basename(model_path_to_evaluate)} - Evaluated on {num_samples_processed_count} samples. Overall Mean AEPE: {mean_aepe_overall:.4f}"
        )
        results_summary[os.path.basename(model_path_to_evaluate)] = {
            "aepe": mean_aepe_overall,
            "samples_evaluated": num_samples_processed_count,
        }
    else:
        print(
            f"Model: {os.path.basename(model_path_to_evaluate)} - No samples processed."
        )
        results_summary[os.path.basename(model_path_to_evaluate)] = {
            "aepe": float("inf"),
...
def evaluate_all_checkpoints(config):
    ...
    torch.manual_seed(0)  # Set seed for deterministic dataset iteration across models
    ...
    if all_results:
        ...
        sorted_results = sorted(
            all_results.items(), key=lambda item: item[1].get("aepe", float("inf"))
        )
        ...
        for model_name, result in sorted_results:
            if "error" in result:
                ...
                print(
                    f"Model: {model_name} - Mean AEPE: {result['aepe']:.4f} (on {result['samples_evaluated']} samples)"
                )
    ...
    if config.NUM_SAMPLES_TO_VISUALIZE > 0:
        print(
            f"Visualizations (if generated) saved to directory: {os.path.dirname(config.OUTPUT_IMAGE_PATH)}"
        )
...
if __name__ == "__main__":
    ...
    # Adjustments for multi-model evaluation config.BATCH_SIZE = 4  # For safety with num_samples_to_evaluate and partial batches
    config.NUM_SAMPLES_TO_EVALUATE = 30  # Reduce for faster testing of multiple models
    config.NUM_SAMPLES_TO_VISUALIZE = (
        1  # Reduce to 1 or 0 to speed up / save disk space
    )

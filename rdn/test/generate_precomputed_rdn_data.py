...
try:
    from rdn.datasets import InpaintingDataset  # For adapting its logic
...
def main():
    ...
    perspective_distorter_transform = T.RandomPerspective(distortion_scale=0.3, p=1.0)

    # --- Load dataset file lists (similar to InpaintingDataset init) ---
    # This part is simplified from InpaintingDataset for clarity here.
    # In a production script, you'd use the robust path finding from InpaintingDataset.
    vimeo_list_file = os.path.join(
        os.path.dirname(gen_config.vimeo_dir), "sep_testlist.txt"
    )
    if not os.path.exists(vimeo_list_file):
        raise FileNotFoundError(f"Vimeo list file not found: {vimeo_list_file}")

    vimeo_sequences_paths_from_list = []
    with open(vimeo_list_file, "r") as f:
        for line in f:
            seq_folder_path = os.path.join(gen_config.vimeo_dir, line.strip())
            frames_in_seq = sorted(glob.glob(os.path.join(seq_folder_path, "im*.png")))
            if len(frames_in_seq) >= gen_config.k_frames:
                vimeo_sequences_paths_from_list.append(frames_in_seq)

    if not vimeo_sequences_paths_from_list:
        raise ValueError("No Vimeo sequences loaded.")
    print(f"Loaded {len(vimeo_sequences_paths_from_list)} Vimeo sequence groups.")
    ...
    for dataset_type, num_samples_to_gen in [
        ("train", gen_config.num_train_samples),
        ("val", gen_config.num_val_samples),
    ]:
        ...
        for i in tqdm(
            ...
            # Randomly pick a Vimeo sequence for each sample
            selected_vimeo_paths = random.choice(vimeo_sequences_paths_from_list)
            ...
            try:
                f_in, i_k_m, b_k = generate_single_sample(
                    ...
                    gen_config,
                    to_tensor_transform,
                    perspective_distorter_transform,
